...
export class ProductService {
  ...
  productsWithCategory$ = combineLatest([
    ...
    shareReplay(1)
    // el sharedReplay me permite tener en cache y emitir la ultima emision del observable a todos
    // los suscriptores del observable colocando en el buffer (1) debido a que es un solo elemento
    // esto mejora la performace y cuando
  );
  ...
  // solo hay delau cuando se lanza la primaera vez
  ...
  // Suppliers for the selected product
  // Only gets the suppliers it needs when it needs
  // El hecho de poner switch map implica que solamente se va a mostrar el ultimo item seleccionado
  // siempre hay delay porque se busca solo lo que se necesita
  ...
  */
  ...
  // Merge the streams
  // aca combinamos el dataStream que trae los productos con la accion de insertar un nuevo producto
  // par que luego el pipe ejecute nuevamente el observable y y transforme
  // luego con el scan lo que hacemos es insertarlo en el array de product  con el operador spread
  // copio todos los elementos en el ...acc y le agrego el value.
